...
function FloatingSearch({close}:{close:()=>void}) {
  ...
  return (
    <div className="min-h-screen w-screen mx-auto bg-black/10    flex flex-col justify-start items-center absolute top-0 left-0  z-100 p-4">
        <div className="w-full my-6 text-sm hover:cursor-pointer flex justify-end items-center pr-28">
  ...
  {result && result.length>0 &&(
    <div className="w-full lg:w-4/5 items-start justify-center">
        ...
        <h3>région</h3>
          ...
          {result.map((record) => (
            <div key={record.NOUVEAU_CODE_POSTAL} className="grid grid-cols-4 p-2 text-sm md:text-base bg-white  text-gray-600">
